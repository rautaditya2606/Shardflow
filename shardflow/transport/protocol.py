...
class TensorMessage:
    ...
    session_id: str
    tensor: Optional[torch.Tensor]  # None for CLEAR messages
...
def encode_message(msg: TensorMessage) -> bytes:
    ...
    if msg.msg_type == MessageType.CLEAR:
        # CLEAR messages have no tensor payload
        payload = header
    else:
        # ACTIVATION / LOGITS: header + tensor_meta + tensor_bytes
        tensor = msg.tensor
        if tensor is None:
            raise ValueError(f"Tensor required for message type {msg.msg_type}")

        # Ensure contiguous for raw bytes
        tensor = tensor.contiguous().cpu()
        tensor_meta = _encode_tensor_meta(tensor)
        # Use raw byte view — works for all dtypes including bfloat16
        # (numpy doesn't support bf16)
        tensor_bytes = bytes(tensor.untyped_storage())
        payload = header + tensor_meta + tensor_bytes
...
def decode_message(data: bytes) -> TensorMessage:
    ...
    if msg_type == MessageType.CLEAR:
        return TensorMessage(msg_type=msg_type, session_id=session_id, tensor=None)

    # Parse tensor metadata
    shape, dtype, offset = _decode_tensor_meta(data, offset)

    # Parse tensor bytes — use torch.frombuffer for all dtypes (including bf16)
    tensor_bytes = data[offset:]
    # Calculate element count from shape
    numel = 1
    for dim in shape:
        numel *= dim
    tensor = torch.frombuffer(bytearray(tensor_bytes), dtype=dtype, count=numel)
    tensor = tensor.reshape(shape).clone()  # clone to own the memory

    return TensorMessage(msg_type=msg_type, session_id=session_id, tensor=tensor)
